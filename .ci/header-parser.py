...
DEFAULT_REPO_URL = "https://github.com/digitalsleuth/winfor-salt"
DEFAULT_PARENT_PATH = "the-tools"  # used to find the page group
DEFAULT_VARIANTID = "main"
DEFAULT_SLS_SEARCH_PATH = f'{os.path.dirname(os.path.realpath(__file__))}/../'
...
def env_or_required(key, help, default=None):
    ...
    d["help"] = "ENV_VAR: {}\n".format(key) + \
    ("Default: {}\n".format(default) if default else "") + help
...
def parse_header(base_path, file_name, repo_url):
            ...
            if not line or line.startswith("{%") or line.startswith("{{"): # can be empty line or template markup
                ...
                if match:
                    ...
                    if key.lower() == "name":
                        ...
                        if not value:
                            raise KeyError(f"categories field empty: {file_path}")
                    ...
                    get_logger().warning(f"file: {file_path} - line does not match with regex: {line}")
    ...
    if not fields:
        ...
        raise KeyError(f"found header but no categories: {file_path}")

    state_file_path = os.path.splitext(file_name)[0].strip('./').replace("/", ".")
    state_file_link = f"{repo_url}/blob/master/{file_name}" # file_name is relative so it should work
