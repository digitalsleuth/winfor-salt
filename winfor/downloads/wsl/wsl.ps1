...
$wslLogFile = "C:\winfor-saltstack-$winforVersion-downloads-wsl.log"
$repoLogFile = "C:\winfor-saltstack-$winforVersion-downloads.log"
$wslErrorLog = "C:\winfor-saltstack-$winforVersion-downloads-wsl-errors.log"
$filePath = "C:\Program Files\Salt Project\Salt\salt-call.exe"
$saltArgs = "-l info --local --retcode-passthrough --log-file=`"$wslLogFile`" --log-file-level=debug --out-file=`"$wslLogFile`" --out-file-append --state-output=mixed state.sls winfor.downloads.wsl.wsl-config pillar=`"{'winfor_user': '_this_user_', 'inpath': '_this_path_', 'wsl_choice': '_this_choice_', 'downloads': '_this_downloads_'}`""
...
Write-Host "[+] Installing _this_distro_ in WSL" -ForegroundColor Green
...
if ($repoFailures -ne 0 -and $repoFailures -ne $null) {
    ...
    $errors = ''
    Write-Host ("`n--- Win-FOR Download Results ---`n$repoLogFile`n$repoResults") -ForegroundColor Yellow
}
if ($wslFailures -ne 0 -and $wslFailures -ne $null) {
    $errors | Out-File $wslErrorLog -Append
    $errors = ''
    Write-Host ("`n--- WSL Download Results ---`n$wslLogFile`n$wslResults") -ForegroundColor Yellow
}
    ...
    Write-Host ("`n--- Win-FOR Download Results ---`n$repoLogFile`n$repoResults") -ForegroundColor Green
    Write-Host ("`n--- WSL Download Results ---`n$wslLogFile`n$wslResults") -ForegroundColor Green
