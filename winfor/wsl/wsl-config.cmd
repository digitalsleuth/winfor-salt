...
:runas
echo $saltArgs = "-l debug --local --retcode-passthrough --log-file=`"C:\winfor-wsl.log`" --log-file-level=debug --out-file=`"C:\winfor-wsl.log`" --out-file-append --state-output=mixed state.sls winfor.wsl.wsl-config pillar=`"{'winfor_user': '_this_user_', 'inpath': '_this_path_'}`""> C:\salt\tempdownload\wsl-after-reboot.ps1
